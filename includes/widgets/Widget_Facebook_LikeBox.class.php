 ...
 */
class Widget_Facebook_LikeBox extends Widget {
	...
	function __construct() {
		parent::__construct(
			'facebook-likebox2',
			apply_filters( 'widget_name', __( 'Facebook Like Box') ),
			array(
				'classname' => 'widget_facebook_likebox',
				'description' => __( 'Display a Facebook Like Box to connect visitors to your Facebook Page')
			)
		);
	}
	...
	function widget( $args, $instance ) {
		...
		$like_bg_colour          = apply_filters( 'fb_likebox_bg', ( 'dark' == $like_args['colorscheme'] ? '#000' : '#fff' ), $like_args['colorscheme'] );
		...
		if ( ! empty( $title ) ) :
			...
			echo apply_filters( 'facebook_likebox_title', $likebox_widget_title, $title, $page_url );
		...
		do_action( 'stats_extra', 'widget', 'facebook-likebox' );
	}
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'show_border' ); ?>">
				...
				<?php _e( 'Show Border'); ?>
				<br />
				<small><?php _e( 'Show a border around the plugin.'); ?></small>
			</label>
		</p>
	...
	function get_default_args() {
		...
		return apply_filters( 'facebook_likebox_defaults', $defaults );
	}
	...
	function guess_locale_from_lang( $lang ) {
		...
		if ( !class_exists( 'GP_Locales' ) ) {
			if ( !defined( '_GLOTPRESS_LOCALES_PATH' ) || !file_exists( _GLOTPRESS_LOCALES_PATH ) ) {
				return false;
			}

			require _GLOTPRESS_LOCALES_PATH;
		}

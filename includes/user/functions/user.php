...
class_alias('\rapidopress\user\User', 'WP_User');
class_alias('\rapidopress\user\User_Query', 'WP_User_Query');
class_alias('\rapidopress\user\Session_Tokens', 'WP_Session_Tokens');
class_alias('\rapidopress\user\User_Meta_Session_Tokens', 'WP_User_Meta_Session_Tokens');
class_alias('\rapidopress\user\PasswordHash', 'PasswordHash');
...
function wp_validate_logged_in_cookie( $user_id ) {
	...
	if ( is_blog_admin()  || empty( $_COOKIE[LOGGED_IN_COOKIE] ) ) {
		return false;
	}
...
function clean_user_cache( $user ) {
	if ( is_numeric( $user ) )
		$user = new \rapidopress\user\User( $user );
...
function wp_insert_user( $userdata ) {
	...
	$user = new \rapidopress\user\User( $user_id );
...
function check_password_reset_key($key, $login) {
	...
	if ( empty( $wp_hasher ) ) {
		$wp_hasher = new \rapidopress\user\PasswordHash( 8, true );
	}
...
function reset_password( $user, $new_pass ) {
	 ...
	 *
	 * @param object $user     The user.
	 * @param string $new_pass New user password.PasswordHash
	 */

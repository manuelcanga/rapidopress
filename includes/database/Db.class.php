...
class Db {
	...
	public function select( $db, $dbh = null ) {
		...
		if ( ! $success ) {
			...
			if ( ! did_action( 'template_redirect' ) ) {
				\rapidopress\database\Errors::cant_select_database($db, $this->user);
			}
			return;
		}
	}
	...
	public function db_connect( $allow_bail = true ) {
		...
		if ( ! $this->dbh && $allow_bail ) {
			\rapidopress\database\Errors::connection_error($this->dbhost);
		} elseif ( $this->dbh ) {
			if ( ! $this->has_connected ) {
				$this->init_charset();
			}
	...
	public function check_connection( $allow_bail = true ) {
		...
		// If template_redirect has already happened, it's too late for wp_die()/\rapidopress\database::db_dead().

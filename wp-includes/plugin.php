...
function apply_filters( $tag, $value ) {
	...
	//first arg is $tag ( it's unuseful ) and second: $value and third, ... are extra params
	...
	do {
		...
		foreach( (array) current($wp_filter[$tag]) as $the_ ) {
			if ( !is_null($the_['function'])  ){
				...
				//change first $arg for new $value;
				$args[0] = $value;
			}
		}
...
function do_action($tag, $arg = '') {
	...
	do {
		foreach ( (array) current($wp_filter[$tag]) as $the_ ) {
			...
			}else if(is_string($the_['function']) && file_exists($the_['function']) ) {
				require($the_['function']);
			}
		}

 ...
 * @access system
 *
 * @return bool if class is found(true) or not(false)
 * 
 */
function rapido_autoload_classes($class_name) {

	$class = explode('\\', $class_name);

	//Removing rapidopress one element
	$place = array_shift($class);

	if($place) {
		$class_file_name = implode('/', $class);
	}

	if(!$place) {
		$class_file = RAPIDO_INCLUDES.$class_name .'.class.php';
	}else if('rapidopress' == $place) 
		$class_file = RAPIDO_INCLUDES.$class_file_name .'.class.php';
	else if('plugins' == $place && define('WP_PLUGIN_DIR') ) 
		$class_file = WP_PLUGIN_DIR.'/'.$class_file_name .'.class.php';
	else if('theme' == $place) 
		$class_file = get_template_directory() .'/'.$class_file_name .'.class.php';
	else
		return ;
	...
	if(file_exists($class_file) ) {
		...
		if(class_exists($class_name, false) && method_exists($class_name, '__initialize') ) {
			$class_name::__initialize();
		}
			return true;
	}
...
spl_autoload_register('rapido_autoload_classes', false);
...
function require_wp_db() {
	...
	return $wpdb = new \rapidopress\database\Db( DB_USER, DB_PASSWORD, DB_NAME, DB_HOST );
}
...
function wp_set_wpdb_vars() {
	...
	if ( !empty( $wpdb->error ) )
		\rapidopress\database\Errors::dead_db();
...
function wp_start_object_cache() {
	...
	if ( ! wp_using_ext_object_cache() )
		require_once ( RAPIDO_INCLUDES . 'system/functions/cache.php' );
		...
		foreach ( $locales as $locale ) {
			foreach ( $locations as $location ) {
				if ( file_exists( $location . '/' . $locale . '.mo' ) ) {
					...
					if ( is_admin() && defined( 'WP_SETUP_CONFIG' ) && file_exists( $location . '/admin-' . $locale . '.mo' ) )

 ...
 * one or the other.
 *
 * Prevents redirection for searches, comment popup, and
...
function redirect_canonical( $requested_url = null, $do_redirect = true ) {
	...
	if ( is_search() || is_comments_popup() || is_admin() || is_preview() || is_robots() || ( $is_IIS && !iis7_supports_permalinks() ) ) {
		return;
	}
	...
	if ( is_404() ) {
		...
		if ( get_query_var('paged') || get_query_var('cpage') ) {
			while ( preg_match( "#/$wp_rewrite->pagination_base/?[0-9]+?(/+)?$#", $redirect['path'] ) || preg_match( "#/{$wp_rewrite->comments_pagination_base}-[0-9]+(/+)?$#", $redirect['path'] ) ) {
				// Strip off paging

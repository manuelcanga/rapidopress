...
/**
 * Retrieve the root url for the current site.
 *
 * Returns the 'root' url 
 *
 * @since 3.0.0
 *
 * @param  string $path   Optional. Path relative to the root url. Default empty.
 * @return string root url link with optional path appended.
*/
function root_url( $path = '') {
	...
	$short_url =  wp_make_link_relative($guess_url);

	$full_url = rtrim($short_url, '/').'/'.ltrim($path, '/');

	//if https then url starts with double slash('//')
	if(0 === strpos($guess_url, 'https://') ) {
		$full_url = '/'.$full_url;
	}
	...
	return 	apply_filters( 'root_url', $full_url, $path);
}

...
class WP_Rewrite {
	...
	/**
	 * Endpoints added by {@link add_rewrite_endpoint()}.
	 *
	 * @since 2.1.0
	 * @access private
	 * @var array
	 ...
	 */
	public function generate_rewrite_rules($permalink_structure, $ep_mask = EP_NONE, $paged = true, $walk_dirs = true, $endpoints = true) {
		//build a parts of URLs
			...
			if ( $num_toks ) {
				...
				if ( $post ) {
					...
					if ( ! $page ) //require <permalink>/attachment/stuff form for pages because of confusion with subpages
						$rewrite = array_merge($rewrite, array($sub1 => $subquery,  $sub1comment => $subcommentquery));
					$rewrite = array_merge(array($sub2 => $subquery, $sub2comment => $subcommentquery), $rewrite);
				}
			} //if($num_toks)
		...
		foreach ( $this->extra_permastructs as $permastructname => $struct ) {
			if ( is_array( $struct ) ) {
				if ( count( $struct ) == 2 )
					...
					$rules = $this->generate_rewrite_rules( $struct['struct'], $struct['ep_mask'], $struct['paged'],  $struct['walk_dirs'], $struct['endpoints'] );
			} else {
